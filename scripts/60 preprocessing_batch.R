...
# count normalization
hypo.list1 <- c(Young_1, Young_3, Aged_1, Aged_3)
...
hypo.list2 <- c(Young_2, Young_4, Aged_2, Aged_4)
...
obj1 <- merge(x = hypo.list1[[1]], y = hypo.list1[c(2:length(hypo.list1))], add.cell.ids = c("young1", "young3", "aged1", "aged3"))
obj1 <- NormalizeData(obj1)
obj1 <- FindVariableFeatures(obj1)
obj1 <- ScaleData(obj1)
obj1 <- RunPCA(obj1, verbose = FALSE)
obj1 <- SCTransform(obj1, variable.features.n = 2000)
obj1 <- FindNeighbors(obj1, dims = 1:30)
obj1 <- RunUMAP(obj1, dims = 1:30)

obj2 <- merge(x = hypo.list2[[1]], y = hypo.list2[c(2:length(hypo.list2))], add.cell.ids = c("young2", "young4", "aged2", "aged4"))
obj2 <- NormalizeData(obj2)
obj2 <- FindVariableFeatures(obj2)
obj2 <- ScaleData(obj2)
obj2 <- RunPCA(obj2, verbose = FALSE)
obj2 <- SCTransform(obj2, variable.features.n = 2000)
obj2 <- FindNeighbors(obj2, dims = 1:30)
obj2 <- RunUMAP(obj2, dims = 1:30)

obj <- merge(x = Young_1, y = c(Young_2, Young_3, Young_4, Aged_1, Aged_2, Aged_3, Aged_4), add.cell.ids = c("Young_1", "Young_2", "Young_3", "Young_4", "Aged_1", "Aged_2", "Aged_3", "Aged_4"))
...
saveRDS(hypo.integrated1, './batch3_test_hypo_integrated_2k.RDS')
saveRDS(hypo.integrated2, './batch3_train_hypo_integrated_2k.RDS')

sparse_integ1 <- hypo.integrated1[['integrated']]@scale.data
sparse_integ2 <- hypo.integrated2[['integrated']]@scale.data

top2k_hvg_1 <- rownames(sparse_integ1)
top2k_hvg_2 <- rownames(sparse_integ2)

top2k_hvg_intersect <- Reduce(intersect, list(top2k_hvg_1,top2k_hvg_2))

write.csv(t(as.matrix(sparse_integ1[top2k_hvg_intersect,])), 'batch3_test_hvg2k_std_integrated.csv')
write.csv(t(as.matrix(sparse_integ2[top2k_hvg_intersect,])), 'batch3_train_hvg2k_std_integrated.csv')

p4 <- DimPlot(hypo.integrated2, cols = c("#25838E", "#D43D4F"), group.by = 'stim') + 
...
p5 <- DimPlot(hypo.integrated1, cols = c("#422CB2", "#85B22C"), group.by = 'batch') + 
  ggtitle('test set -- colored by batches')+
  theme(plot.title = element_text(hjust = 0.5)) + NoAxes()

p6 <- DimPlot(hypo.integrated2, cols = c("#422CB2", "#85B22C"), group.by = 'batch') + 
  ggtitle('training set -- colored by batches')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoLegend() + NoAxes()

p6|p5 

p1 <- DimPlot(obj1, cols = c("#25838E", "#D43D4F"), group.by = 'stim') + 
  ggtitle('test set -- before integration')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoAxes()

p3 <- DimPlot(obj2, cols = c("#25838E", "#D43D4F"), group.by = 'stim') + 
  ggtitle('training set -- before integration')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoLegend() + NoAxes()

p3|p1 

p7 <- DimPlot(obj1, cols = c("#422CB2", "#85B22C"), group.by = 'batch') + 
  ggtitle('test set -- before, by batches')+
  theme(plot.title = element_text(hjust = 0.5)) + NoAxes()

p8 <- DimPlot(obj2, cols = c("#422CB2", "#85B22C"), group.by = 'batch') + 
  ggtitle('training set -- before, by batches')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoLegend() + NoAxes()

p8|p7 

p11 <- DimPlot(obj, cols = c("#25838E", "#D43D4F"), group.by = 'stim') + 
  ggtitle('before integration')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoAxes()

p12 <- DimPlot(obj, cols = c("#422CB2", "#85B22C"), group.by = 'batch') + 
  ggtitle('before integration')+
  theme(plot.title = element_text(hjust = 0.5)) +
  NoAxes()
